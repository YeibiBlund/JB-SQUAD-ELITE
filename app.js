...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const btnRivalsHistory = document.getElementById('btn-rivals-history');
    const btnBackToSessionsFromRivals = document.getElementById('btn-back-to-sessions-from-rivals');
    const rivalsSearchInput = document.getElementById('rivals-search');
            ...
            <div style="display: flex; flex-direction: column; gap: 10px;">
                ...
                <div style="display: flex; flex-direction: column; gap: 8px; max-height: 420px; overflow-y: auto; padding-right: 4px;">
                    ${sortedMatches.map(m => {
                        ...
                        const isVisitor = m.matchCondition === 'visitor';
                        const conditionLabel = isVisitor ? 'Visitante' : 'Local';
                        const typeLabel = m.type === 'official' ? 'Oficial' : 'Amistoso';

                        // Inversión estética del marcador para la UI en base a la localía
                        const displayHomeScore = isVisitor ? m.scoreAway : m.scoreHome;
                        const displayAwayScore = isVisitor ? m.scoreHome : m.scoreAway;
