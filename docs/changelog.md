...
- **Prevención de Saltos de Línea y Truncado con Elipsis**: Forzado `white-space: nowrap !important; overflow: hidden !important; text-overflow: ellipsis !important;` en todas las celdas de las filas (`.rival-row td`) y cabeceras (`.th-rivals-sortable`). Esto garantiza que los nombres largos de rivales o la inyección de iconos de ordenación (`▲`/`▼`) nunca fuercen saltos de línea ni descoloquen las columnas, manteniendo la visualización 100% estática y profesional.
